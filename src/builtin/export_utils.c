/* ************************************************************************** */
/*                                                                            */
/*                                                        :::      ::::::::   */
/*   export_utils.c                                     :+:      :+:    :+:   */
/*                                                    +:+ +:+         +:+     */
/*   By: iostancu <iostancu@student.42.fr>          +#+  +:+       +#+        */
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2024/09/16 18:43:06 by iostancu          #+#    #+#             */
/*   Updated: 2024/09/16 18:43:06 by iostancu         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "minishell.h"

int	check_name_syntax(char *name)
{
	int	i;

	i = -1;
	while (name[++i])
	{
		if (!ft_isalnum(name[i]) && name[i] != '_')
			return (1);
	}
	return (0);
}

char	**restore_equals(char **var)
{
	int		i;
	char	*old_value;

	i = 1;
	while (var[++i])
	{
		old_value = var[1];
		var[1] = ft_strjoin(var[1], var[i]);
		free(old_value);
	}
	return (var);
}

int	ckeck_syntax2(char **new_vars, int i, char **split_var)
{
	if (ft_strchr(new_vars[i], '=') != ft_strrchr(new_vars[i], '='))
		split_var = restore_equals(split_var);
	if (!split_var[0] || !split_var[1] || !ft_isalpha(*split_var[0])
		|| check_name_syntax(split_var[0]))
	{
		ft_printf("minishell: export: `%s': is not a valid identifier\n",
			new_vars[i]);
		(free_dp(split_var), *new_vars[i] = '?');
		return (1);
	}
	return (0);
}

int	check_syntax(t_list **env, char **new_vars)
{
	int			i;
	int			new_lines;
	char		**split_var;
	char		value_buf[BUF_SIZE];

	i = -1;
	new_lines = 0;
	while (new_vars[++i])
	{
		if (!ft_strchr(new_vars[i], '='))
		{
			*new_vars[i] = '?';
			continue ;
		}
		split_var = ft_split(new_vars[i], '=');
		if (ckeck_syntax2(new_vars, i, split_var))
			continue ;
		if (ft_getenv(env, split_var[0], value_buf))
			(replace_var(env, new_vars[i], ft_strjoin(split_var[0], "=")));
		else
			new_lines++;
		free_dp(split_var);
	}
	return (new_lines);
}
