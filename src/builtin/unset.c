...
int	exec_unset(t_cmd *cmd, t_pipe *data, int pos)
{
	...
	{
		uset.j = 0;
		uset.arr_size = get_array_size(data->env_mini);
		if (is_correct_env_variable(cmd->scmd[pos]->args[uset.k], "unset")
			&& env_var_already_exist(data->env_mini,
				cmd->scmd[pos]->args[uset.k]))
		{
			...
			free_memory((const char **)data->env_mini, uset.arr_size);
			data->env_mini = uset.tmp;
		}
		uset.k++;
	}
	return (EXIT_SUCCESS);
...
static void	unset_variables(t_pipe *data, t_cmd *cmd, t_unset *uset, int pos)
{
	...
	uset->i = 0;
	while (data->env_mini[uset->i])
	{
		var = get_env_variable(data->env_mini[uset->i]);
		if (f_strict_strncmp(var, cmd->scmd[pos]->args[uset->k],
				f_strlen(cmd->scmd[pos]->args[uset->k])) != 0)
		{
			uset->tmp[uset->j] = f_strdup(data->env_mini[uset->i]);
			uset->j++;
		}
		free(var);
		uset->i++;
